...
int syscall_full_except(regs_t *r, uint32_t spsr, uint32_t pc) {
    // pass seperately so can sanity check
    // if (spsr != r->regs[REGS_CPSR]) {
    //     output("spsr is %x, expected: %x, REGS_CPSR: %d\n", spsr, r->regs[REGS_CPSR], REGS_CPSR);
    // }
    assert(spsr == r->regs[REGS_CPSR]);
    
    // if (pc != r->regs[REGS_PC]) {
    //     output("pc is %x, expected: %x, REGS_PC: %d\n", pc, r->regs[REGS_PC], REGS_PC);
    // }
    assert(pc == r->regs[REGS_PC]);
